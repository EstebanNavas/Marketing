...
public interface TblDctosOrdenesRepo extends JpaRepository<TblDctosOrdenes, Integer> {
	...
	// Modificamos el IDTIPOORDEN de 67 a 17
		...
		// Modificamos el ESTADO a 1 Cuando se finaliza por completo la pqr 
		  ...
		  @Transactional
		  @Query(value = "UPDATE tblDctosOrdenes                                  "
	                + " SET vrPagoUltimo = tmpPAG.vrPago,                      "
	                + "     fechaPagoUltimo = tmpPAG.fechaPago                 "
	                + " FROM tblDctos                                          "
	                + " INNER JOIN tblDctosOrdenes                             "
	                + " ON tblDctos.IDLOCAL = tblDctosOrdenes.IDLOCAL          "
	                + " AND tblDctos.IDTIPOORDEN = tblDctosOrdenes.IDTIPOORDEN "
	                ...
	                + " 	    tblPagos.nitCC,                                "
	                + " 	    tblPagos.vrPago,                               "
	                + " 	    tblPagos.fechaPago) ) AS tmpHIS                "
	                + " ON tblDctos.idLocal          = tmpHIS.idLocal          "
	                + " AND tblDctos.idTipoOrden     =  tmpHIS.idTipoOrden     "
	                + " AND tblDctos.idCliente       =  tmpHIS.idCliente       "
	                + " AND tblDctosPeriodo.idPeriodo=  tmpHIS.idPeriodo       "
	                + " WHERE tblDctos.idTipoOrden   = 9                       "
	                + " AND   tblDctos.idLocal     =                           "
	                + "?1                                         "
	                + " AND NOT tmpHIS.idPeriodo IS NULL                       "
	                + " GROUP BY tblDctos.idLocal,                             "
	                + "         tblDctos.idTipoOrden,                          "
	                + " 		tblDctos.idOrden,                          "
	                + " 		tblDctos.idCliente,                        "
	                + " 		tblDctosPeriodo.idPeriodo,                 "
	                + " 		tmpHIS.vrPago,                             "
	                + " 		tmpHIS.fechaPago) AS tmpPAG                "
	                + " ON tblDctosOrdenes.idLocal = tmpPAG.IDLOCAL            "
	                + " AND tblDctosOrdenes.idTipoOrden = tmpPAG.idTipoOrden   "
