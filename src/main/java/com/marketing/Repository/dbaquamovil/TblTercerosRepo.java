	  ...
	  @Query(
		...
		// Modificamos el IDTIPOORDEN de 67 a 17
		  ...
		  @Query(value = "UPDATE tblTerceros SET nombreTercero = ?1, direccionTercero = ?2, direccionCobro = ?3, idDptoCiudad = ?4, telefonoFijo = ?5, telefonoCelular = ?6, " +
				  		...
				  		"fechaInstalacionMedidor = ?16, codigoAlterno = ?17, tipoSuscriptor = ?18, matricula = ?19 "+
		                 "WHERE tblTerceros.idLocal = ?20 " +
		                 "AND tblTerceros.idCliente = ?21 " +
		                 "AND tblTerceros.idTipoTercero = ?22 ", nativeQuery = true)
