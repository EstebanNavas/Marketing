...
public interface TblDctosRepo extends JpaRepository<TblDctos, Integer> {
	...
	@Query("SELECT MAX(r.idDcto) FROM TblDctos r " +
	  ...
	  @Query(value = "SELECT (vrBase + vrIva) AS vrTotal " +
              "FROM bdaquamovil.dbo.tblDctos " +
              "WHERE tblDctos.IDLOCAL = ?1 " +
              "AND tblDctos.idDcto = ?2 " +
              "AND tblDctos.idCliente = ?3 ",
              nativeQuery = true)
	  Integer ObtenerVrFra(int idLocal, int idDcto, String idCliente);
	  ...
	  @Query(value = "INSERT INTO tbldctos (idLocal,             "
              ...
              + "?35,"
              + "?36)", nativeQuery = true)
	  public void ingresaDctoDctoSoporte(int idLocal, int IdTipoOrden, int IdOrden, int IdDcto, int Indicador, String FechaDctoSqlServer, Double VrBaseSinRedondeo, Double VrPago, int Estado, Double VrIva, 
			 int IdTipoNegocio, Double VrRteFuente, Double VrDescuento, Double VrRteIva, Double VrRteIca, String NombreTercero, int IdUsuario, String IdCliente, int DiasPlazo, int PorcentajeDscto, 
			 int IdCausa, String IdDctoNitCC, String FechaDctoNitCCSqlServer, int VrPagarDctoNitCC, int VrDsctoFcro, Double VrCostoMV, int IdLocalCruce, int IdTipoOrdenCruce, int IdDctoCruce, 
			 int IdPeriodo, int IdVendedor, Double VrImpoconsumo, Double VrCostoIND, int IdOrdenCruce, int EtapaSTR, int EnvioFE);
...
//				+ "      (tblPagos.fechaPago) AS fechaDctoSiigo,                       "          
//				+ "       'COP' AS  SiglaMoneda,                                       "          
//				+ "       MAX(tblPlus.cuentaRecaudoDebito)                             "          
//				+ "                         AS idSubcuenta,                            "          
//				+ "       MAX(tblTerceros.CC_Nit) AS idCliente,                        "          
//				+ "       CAST(ROUND(SUM(tblPagosMedios.vrMedio),2)                    "          
//				+ "                 AS DECIMAL(10,2))  AS vrDebito,                    "          
...
//				+ "     (tblPagos.fechaPago) AS fechaElaboracion,                      "          
//				+ "      'COP' AS  SiglaMoneda,                                        "          
//			      + "   MAX(tblPlus.cuentaRecaudoCredito)                              "          
//		          + "                                                                  "          
//		          + "                                                                  "          
//				+ "     AS codCuentaContable, 					                      "          
//				+ "     MAX(tblTerceros.CC_Nit) AS idCliente,                          "          
...
//				+ "     ORDER BY 2 ASC,8 DESC                                          ",
	  ...
	  @Modifying
	  @Transactional
	  @Query(value = " UPDATE bdaquamovil.dbo.tblDctos set idDctoCruce = ?1 "
              + " WHERE idlocal = ?2    "
              + " AND IDTIPOORDEN = ?3  "
              + " AND idDcto = ?4       ",
              nativeQuery = true)
	  public void actualizaDctoCtruceNE(int xIdDctonNew, int idLocal, int IdTipoOrden, int IdDcto);
	  ...
	  @Query(value = " WITH Periodos AS (                                                 "
			  ...
			  + "        od.longitud,                                                "
			  + "        ROW_NUMBER() OVER (                                         "
			  + "            PARTITION BY d.idPeriodo, od.idCliente                  "
			  + "            ORDER BY od.fechaRegistroTx DESC                        "
			  + "        ) AS rn                                                     "
			  + "    FROM tblDctos d                                                 "
			  + "    INNER JOIN tblDctosOrdenesDetalle od                            "
			  + "        ON d.IDLOCAL = od.IDLOCAL                                   "
			  + "       AND d.IDTIPOORDEN = od.IDTIPOORDEN                           "
			  + "       AND d.IDORDEN = od.IDORDEN                                   "
			  + "    WHERE d.IDLOCAL = ?1                                           "
			  + "      AND d.IDTIPOORDEN = 9                                         "
			  + "      AND od.idtipo = 4                                             "
			  + "      AND od.idCliente = ?3                                     "
			  + "      AND d.idPeriodo IN (SELECT idPeriodo FROM Periodos)           "
			  + " )                                                                  "
			  + " SELECT                                                             "
			  + "    idCliente,                                                      "
			  + "    fechaRegistroTx,                                                "
	  ...
	  @Query(value = " 	UPDATE tblDctos    			"
			  ...
			  + " AND idCliente = ?4									",
              nativeQuery = true)
	  public void actualizaEtapaSTRxDcto(int idLocal, int idPeriodo, int IdDcto, String idCliente);
	  ...
	  @Query(value = "    SELECT tblDctos.IDLOCAL                                                         "              
		        ...
		        + "    AND tblTerceros.idCliente     =  tblDctos.idCliente                             "              
		        + "    INNER JOIN tblTercerosRuta                                                      "              
		        + "    ON tblTercerosRuta.idLocal    =   tblTerceros.IDLOCAL                           "              
		        + "    AND tblTercerosRuta.idRuta    =   tblTerceros.idRuta                            "              
		        + "    where tblDctos.idLocal   =  ?1                                                  "              			           
				+ "	AND tblDctos.idPeriodo BETWEEN ?2 AND ?3                                           "
		        + "    AND tblDctos.idTipoOrden  IN (9)                                                "              
		        + "    AND tblDctos.idCliente=?4                                                       "              
		        + "    AND EXISTS ( SELECT *                                                           "              
		        + "    			 FROM   tblDctosOrdenesDetalle                                         "            
		        + "    			  WHERE  tblDctos.IDLOCAL      =  tblDctosOrdenesDetalle.IDLOCAL       "            
		        + "    			  AND tblDctos.IDTIPOORDEN =  tblDctosOrdenesDetalle.IDTIPOORDEN       "            
		        + "    			  AND tblDctos.IDORDEN     =  tblDctosOrdenesDetalle.IDORDEN           "            
		        ...
		        + "    where tblDctos.idLocal   =  ?1                                                  "                  
				+ "    AND tblDctos.idPeriodo BETWEEN ?2 AND ?3                                        "
		        + "    AND tblDctos.idTipoOrden  IN (29)                                               "              
		        + "    AND tblDctos.idCliente=?4                                                       "              
		        + "    AND EXISTS ( SELECT *                                                           "              
		        + "    			 FROM   tblDctosOrdenesDetalle                                         "            
		        + "    			  WHERE  tblDctos.IDLOCAL      =  tblDctosOrdenesDetalle.IDLOCAL       "            
		        ...
		        + "    	  ,tblDctos.nombreTercero                                                      "            
		        + "    	  ,tblTercerosRuta.nombreRuta                                                  "            
		        + "          ,tblPagos.fechaPago AS [fechaDcto]                                        "              
		        + "          ,tblPagos.[vrPago]*(-1) AS vrBase                                         "              
		        + "    	  ,tblAgendaLogVisitas.IDLOG                                                   "            
		        + "          ,'Pago' AS tipoDcto                                                       "              
		        + "    FROM tblPagos                                                                   "              
		        + "    INNER JOIN tblAgendaLogVisitas                                                  "              
		        + "    ON tblPagos.IDLOCAL = tblAgendaLogVisitas.idLocal                               "              
		        + "    AND tblPagos.IDLOG = tblAgendaLogVisitas.IDLOG                                  "              
		        + "    INNER JOIN tblDctos                                                             "              
		        + "    ON tblPagos.IDLOCAL = tblDctos.idLocal                                          "              
		        + "    AND tblPagos.idDcto = tblDctos.idDcto                                           "              
		        + "    INNER JOIN tblTerceros                                                          "              
		        + "    ON tblTerceros.idLocal        =  tblDctos.IDLOCAL                               "              
		        + "    AND tblTerceros.idCliente     =  tblDctos.idCliente                             "              
		        + "    INNER JOIN tblTercerosRuta                                                      "              
		        ...
		        + "    where tblPagos.idLocal   =  ?1                                                  "                
				+ "    AND tblDctos.idPeriodo BETWEEN ?2 AND ?3                                        "
		        + "    AND tblPagos.idTipoOrden  IN (9)                                                "              
		        + "    AND tblPagos.[nitCC]=?4                                                         "              
		        + "    ORDER BY tblAgendaLogVisitas.IDLOG                                              ",
              nativeQuery = true)
	  List<TblDctosDTO> listaEstadoCuentaXNuid(int idLocal, int idPeriodoDesde, int idPeriodoHasta, String idCliente );
	  ...
	  @Query(value = "               SELECT tblDctos.IDLOCAL                                                                       "          
			  ...
			  + "               ,tblDctos.fechaDcto                                                                           "          
			  + "               ,tblDctos.vrPago                                                                              "          
			  + "               ,tblDctos.fechaPagoInicio                                                                     "          
			  + "               ,tblDctos.fechaPagoFin                                                                        "          
			  + "               ,tblDctosOrdenes.fechaInicioContrato                                                          "          
			  + "               ,tblDctosOrdenes.vrSalarioBasico                                                              "          
			  + "               ,tblDctosOrdenes.vrSubsidioTransporte                                                         "          
			  + "               ,ISNULL (tblDctosOrdenes.fechaFinContrato, '2099-12-31 00:00:00.000') AS  fechaFinContrato    "            
			  + "               ,tblDctosOrdenes.idContrato                                                                   "          
			  + "               ,ISNULL (tblDctosOrdenes.idMedio, 1) AS idMedio                                               "          
			  + "               ,ISNULL (tblDctosOrdenes.entidadMedio, '.') AS  entidadMedio                                  "          
			  + "               ,ISNULL (tblDctosOrdenes.cuentaMedio, 11111) AS   cuentaMedio                                   "          
			  + "               ,tblDctosOrdenesDetalle.idPlu                                                                 "          
			  + "               ,tblDctosOrdenesDetalle.CANTIDAD                                                              "          
			  + "               ,tblDctosOrdenesDetalle.VRVENTAUNITARIO                                                       "          
			  + "               ,tblDctosOrdenesDetalle.NOMBREPLU                                                             "          
			  ...
			  + "               ,DATEDIFF(day,                                                                                "          
			  + "                tblDctos.fechaPagoInicio,                                                                    "          
			  + "                tblDctos.fechaPagoFin) AS diasContrato                                                       "          
			  + "               ,tblPlus.idPluNE                                                                              "          
			  + "               ,tmpRES.vrDevengado                                                                           "          
			  + "               ,tmpRES.vrDeduccion                                                                           "          
			  + "               FROM bdaquamovil.dbo.tblDctos                                                                 "          
			  ...
			  + "               AND tblDctos.idOrden    =                                                                     "          
			  + "                       tblDctosOrdenes.idOrden                                                               "          
			  + "               INNER JOIN bdaquamovil.dbo.tblPlus                                                            "          
			  + "               ON tblPlus.idLocal     =                                                                      "          
			  + "                    tblDctosOrdenesDetalle.idLocal                                                           "          
			  + "               AND tblPlus.idPlu      =                                                                      "          
			  + "                      tblDctosOrdenesDetalle.idPlu                                                           "          
			  + "               INNER JOIN bdaquamovil.dbo.tblTerceros                                                        "          
			  + "               ON tblTerceros.idLocal         =                                                              "          
			  + "                             tblDctos.idLocal                                                                "          
			  + "               AND tblTerceros.idCliente      =                                                              "          
			  + "                          tblDctos.idCliente                                                                 "          
			  + "              INNER JOIN bdaquamovil.dbo.tblCiudades                                                         "          
			  + "              ON tblCiudades.idCiudad         =                                                              "          
			  + "                        tblTerceros.idDptoCiudad                                                             "          
			  + "             INNER JOIN ( SELECT tmpTOT.IDLOCAL,                                                             "          
			  + "               tmpTOT.IDTIPOORDEN,                                                                           "          
			  + "               tmpTOT.IDORDEN,                                                                               "          
			  + "               SUM(tmpTOT.vrDevengado)                                                                       "          
			  + "                        AS vrDevengado,                                                                      "          
			  + "               SUM(tmpTOT.vrDeduccion)                                                                       "          
			  + "                        AS vrDeduccion                                                                       "          
			  + "             FROM ( SELECT tblDctos.IDLOCAL,                                                                 "          
			  + "               tblDctos.IDTIPOORDEN,                                                                         "          
			  + "               tblDctos.IDORDEN,                                                                             "          
			  + "               CASE                                                                                          "          
			  + "               WHEN tblPlus.idCategoria IN (1400)                                                            "          
			  + "               THEN                                                                                          "          
			  + "               tblDctosOrdenesDetalle.VRVENTAUNITARIO                                                        "          
			  + "               ELSE 0                                                                                        "          
			  + "               END AS vrDevengado,                                                                           "          
			  + "               CASE                                                                                          "          
			  + "               WHEN tblPlus.idCategoria IN (1800)                                                            "          
			  + "               THEN                                                                                          "          
			  + "               tblDctosOrdenesDetalle.VRVENTAUNITARIO                                                        "          
			  + "               ELSE 0                                                                                        "          
			  + "               END AS vrDeduccion                                                                            "          
			  ...
			  + "             AND tblDctos.idDcto           =                                                                 "          
			  + "            ?3                                                                                               "          
			  + "             AND tblPlus.idCategoria IN (1400, 1800) )                                                        "          
			  + "                                          AS tmpTOT                                                          "          
			  + "             GROUP BY tmpTOT.IDLOCAL,                                                                        "          
			  + "               tmpTOT.IDTIPOORDEN,                                                                           "          
			  + "               tmpTOT.IDORDEN) AS tmpRES                                                                     "          
			  + "             ON tblDctos.idLocal        =                                                                    "          
			  + "                                     tmpRES.idLocal                                                          "          
			  + "             AND tblDctos.idTipoOrden   =                                                                    "          
			  + "                                 tmpRES.idTipoOrden                                                          "          
