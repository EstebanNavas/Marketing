...
//	            List<TercerosDTO> alista =  tblTercerosService.listaUnClienteWhatsApp(idLocal, idPeriodoInt, listaIdClientes, idEvento);
	...
	@ResponseBody
	public ResponseEntity<Map<String, Object>> EnviarWhatsAppFacturasRuta(@RequestBody Map<String, Object> requestBody, HttpServletRequest request,Model model) throws JRException, IOException, SQLException {
	    Ctrlusuarios usuario = (Ctrlusuarios) request.getSession().getAttribute("usuarioAuth");
	    Integer IdUsuario = usuario.getIdUsuario();
...
//	            List<TercerosDTO> alista =  tblTercerosService.listaUnClienteWhatsApp(idLocal, idPeriodoInt, listaIdClientes, idEvento);
	            ...
	            //Obtenemos todos los clientes que NO se le hayan enviado Wpp y que el estado Email sea activo =  1
	            List<TercerosDTO>  todosClientes = tblTercerosService.listaTodosLosClientesEstadoFacturaActWhasAppRUTA(idLocal, idPeriodoInt, idRutaInt);
